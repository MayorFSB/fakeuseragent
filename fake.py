...
from fakeuseragent import settings
from fakeuseragent.errors import FakeUserAgentError
from fakeuseragent.log import logger
from fakeuseragent.utils import load, load_cached, str_types, update
...
class FakeUserAgent:
    ...
    def __init__(
            ...
            fallback=None,
            browsers=["chrome", "edge", "internet explorer", "firefox", "safari", "opera"], # noqa
            verify_ssl=True, safe_attrs=tuple()
        ):
        assert isinstance(use_external_data, bool), "use_external_data must be True or False"
        ...
        assert isinstance(safe_attrs, (list, set, tuple)), "safe_attrs must be list,tuple,set of strings or unicode"
        if safe_attrs:
            str_types_safe_attrs = [isinstance(attr, str_types) for attr in safe_attrs]
            assert all(str_types_safe_attrs), "safe_attrs must be list,tuple,set of strings or unicode"
        ...
        try:
            with self.load.lock:
                if self.use_external_data:
                    ...
                    self.data_browsers = load_cached(self.cache_path, self.browsers, verify_ssl=self.verify_ssl)
                else:
                    # By default, we will try to load our local file
                    self.data_browsers = load(self.browsers, verify_ssl=self.verify_ssl)
    ...
    def update(self, use_external_data=None):
        with self.update.lock:
            ...
            if self.use_external_data:
                update(self.cache_path, self.browsers, verify_ssl=self.verify_ssl)
            self.load()
