        ...
        try:
            if urlopen_has_ssl_context:
                ...
                with contextlib.closing(
                        request.urlopen(requestObj, timeout=settings.HTTP_TIMEOUT, context=context)) as response:
                    return response.read()
            else:  # ssl context is not supported
                with contextlib.closing(request.urlopen(requestObj, timeout=settings.HTTP_TIMEOUT)) as response:
                    return response.read()
        except (URLError, OSError) as exc:
            logger.debug("Error occurred during fetching %s", url, exc_info=exc)
...
def get_browser_user_agents_online(browser, verify_ssl=True):
    ...
    """
    html = get(settings.BROWSER_BASE_PAGE.format(browser=quote_plus(browser)), verify_ssl=verify_ssl)
    try:
    ...
    if not browsers:
        raise FakeUserAgentError("No browser user-agent strings found for browser: {browser}".format(browser=browser))
    return browsers
...
def load(browsers, use_local_file=True, verify_ssl=True):
    ...
    if use_local_file:
        try:
            json_lines = ilr.files("fakeuseragent").joinpath("browsers.json").read_text()
    ...
    if fetch_online:
        try:
            ...
            for browser_name in browsers:
                ...
                data[browser_name] = get_browser_user_agents_online(browser_name, verify_ssl=verify_ssl)
        except Exception:
...
from fakeuseragent import settings  # noqa # isort:skip
from fakeuseragent.errors import FakeUserAgentError  # noqa # isort:skip
